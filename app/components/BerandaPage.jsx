...
const DetailCard = ({ icon, consumptionValue, consumptionUnit, emissionValue, emissionUnit, colorClass }) => (
    <div className="bg-white p-3 md:p-4 rounded-xl border shadow-sm flex flex-col justify-between h-full">
        <div className="flex items-start gap-3">
            <div className={`flex-shrink-0 w-9 h-9 md:w-10 md:h-10 rounded-lg flex items-center justify-center ${colorClass.bg}`}>
                <div className={colorClass.text}>{icon}</div>
            </div>
            <div className="min-w-0">
                <p className="text-base md:text-lg font-bold text-slate-800 truncate">{consumptionValue} <span className="text-sm font-normal">{consumptionUnit}</span></p>
                <p className="text-xs md:text-sm text-slate-500 -mt-0.5">Total Konsumsi Bulan Ini</p>
            </div>
        </div>
        <div className="text-right mt-3">
            <p className="text-base font-bold text-slate-700">{emissionValue.toFixed(2)} <span className="text-xs md:text-sm font-medium">{emissionUnit}</span></p>
        </div>
    </div>
);
...
export default function BerandaPage({ user, dataVersion, initialBusinessName }) {
    ...
    return (
        <div className="space-y-8">
            <div
                className="relative p-4 md:p-8 rounded-2xl text-white bg-cover bg-center min-h-[140px] md:min-h-[180px] flex flex-col justify-between"
                style={{ backgroundImage: `linear-gradient(to top, rgba(0,0,0,0.6), rgba(0,0,0,0.1)), url('${userBgImage}')` }}
            >
                <Link href="/dashboard/notifikasi" className="absolute top-4 right-4 w-10 h-10 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center text-white hover:bg-white/30 transition-colors z-10">
                    <BellIcon />
                </Link>

                <div className="relative z-0">
                    {/* --- PERUBAHAN 5: Hapus ternary 'loading ?' --- */}
                    <h1 className="text-2xl md:text-4xl font-extrabold drop-shadow-md">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-8 space-y-6">
                    ...
                    {loading ? (
                        ...
                        <div className="bg-[#22543d] text-white p-6 rounded-xl shadow-lg">
                            ...
                            <p className="text-2xl md:text-3xl font-extrabold">{(summary?.total_all || 0).toFixed(2)} <span className="text-lg md:text-xl font-medium">ton CO₂e</span></p>
                ...
                <div className="lg:col-span-4 bg-white p-6 rounded-xl border shadow-sm flex flex-col justify-between">
                    ...
                    <div className="mt-4">
                        <Link href="/dashboard/laporan-emisi" className="w-full py-3 text-base font-semibold text-white bg-[#22543d] rounded-lg transition-colors hover:bg-[#1c4532] flex items-center justify-center gap-2">
                            <DocumentChartBarIcon />
                            Lapor Emisi Sekarang
                        </Link>
                        <Link href="/dashboard/panduan" className="w-full mt-3 text-sm font-medium text-slate-500 hover:text-[#22543d] flex items-center justify-center gap-2 transition-colors">
