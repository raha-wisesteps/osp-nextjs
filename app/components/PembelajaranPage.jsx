...
import { PlayCircleIcon, DocumentTextIcon, VideoCameraIcon, BookOpenIcon, ArrowLeftIcon } from './Icons';
...
const CATEGORIES = [
    {
        id: 'dasar',
        name: 'Dasar & pemahaman umum',
        icon: <BookOpenIcon /> // Menggunakan ikon yang sudah ada
    },
    {
        id: 'metodologi',
        name: 'Metodologi dan Panduan Teknis',
        icon: <DocumentTextIcon /> // Menggunakan ikon yang sudah ada
    },
    {
        id: 'strategi',
        name: 'Strategi & Tindakan Pengurangan Emisi',
        icon: <PlayCircleIcon /> // Menggunakan ikon yang sudah ada
    },
    {
        id: 'kebijakan',
        name: 'Kebijakan & Kerangka Nasional / Global',
        icon: <BookOpenIcon /> // Menggunakan kembali ikon yang sudah ada
    },
    {
        id: 'glosarium',
        name: 'Glosarium & Tanya Jawab Teknis',
        icon: <DocumentTextIcon /> // Menggunakan kembali ikon yang sudah ada
    }
];
...
export default function PembelajaranPage({ userRole }) {
    const router = useRouter();
    const [resources, setResources] = useState([]);
    const [loading, setLoading] = useState(true);
    const [activeTab, setActiveTab] = useState(CATEGORIES[0].id); // Otomatis mengambil id 'dasar'
    ...
    useEffect(() => {
        const fetchResources = async () => {
            ...
            const { data, error } = await supabase
                .from('learning_materials') // Pastikan nama tabel ini benar
                .select('*')
    ...
    const getFilteredResources = () => {
        // Menggunakan ID baru untuk filter
    ...
    const handleResourceSelect = (resource) => {
        if (resource.type === 'video') {
            router.push(`/dashboard/pembelajaran/${resource.id}`);
        } else {
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8">
            ...
            {userRole === 'admin' && (
                <Link
                    href="/dashboard/admin/pembelajaran"
